...
// Extract keys from the API configuration excluding 'baseUrl'
type TKeysOfUnion<T> = T extends T ? Exclude<keyof T, "baseUrl"> : never;

export function fetcher<KeyType extends TKeysOfUnion<typeof api[typeof module]>>(
  key: KeyType,
...
export function useRequestMutation<Data = any, Error = any>(
  ...
  const k: Key = typeof key === "function" ? key() : (api[module] as any)[key];
  const fetcher = (url: string, { arg }: { arg: TTriggerArgs }) => {
    if (arg.cacheOnly) {
      return Promise.resolve();
    }
